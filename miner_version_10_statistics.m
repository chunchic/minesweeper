% ALWAYS CHOOSE "CHANGE FOLDER"

function miner_version_10_statistics
...
global display_animations_value
global play_sound_value
global show_tips_value
global always_continue_saved_game_value
global always_save_game_on_exit_value
global question_marks

open_options_file = fopen('game_options.txt');
options_data = fscanf(open_options_file,'%i');
length = options_data(1); 
height = options_data(2); 
mines = options_data(3);
   
    ongoing_game_file = fopen('ongoing_game.txt');
    open_loading_board = fopen('save_minesweeper.txt');
    loading_game = fscanf(ongoing_game_file,'%i');
    if ~isempty(loading_game) && ~isempty(open_loading_board)
        loading_game_check = questdlg('Load previous game?','Loading game','Yes','No','No');
        switch loading_game_check
            case 'Yes'
                loading = 1;
                length = loading_game(1);
                height = loading_game(2);
                mines = loading_game(3);
                time = loading_game(4);
                steps = loading_game(5);
                flagged_squares = mines - loading_game(6);
                opened_squares = loading_game(7);
            case 'No'
                loading = 0;
                time = 0;
                steps = 0;
    ...
    display_animations_value = 0;
    play_sound_value = 0;
    show_tips_value = 0;
    always_continue_saved_game_value = 0;
    always_save_game_on_exit_value = 0;
    question_marks = 1;
        
    f = figure('units', 'pixels', 'outerposition', [200 500-(height+5)*button_size (length+3)*button_size (height+7)*button_size],'numbertitle','off','name','Minersweeper', 'menubar', 'none','color',figure_color,'closerequestfcn',@close_game);
    ...
    stats = uimenu(menubar_1,'text','Statistics','menuselectedfcn',@statistics);
    ...
    if loading
        l_b = fscanf(open_loading_board,'%i',[length,height]);
        loading_board = flip(l_b',1);
        for i = 1:length 
            for j = 1:height
                switch loading_board(j,i)
                    case -3
                        board(i,j).String = '?';
                    case -2
                        board(i,j).BackgroundColor = [0 0 0];
                    case 0
                        board(i,j).BackgroundColor = [0.8 0.8 0.8];
                        board(i,j).UserData = '0';
                    case 9
                        board(i,j).UserData = 'BOMB';
                end
                if loading_board(j,i) == -1
                    board(i,j).UserData = '0';
                    board(i,j).BackgroundColor = button_color;
                elseif ~mod(loading_board(j,i),10) && loading_board(j,i) ~= 0
                    board(i,j).UserData = num2str(loading_board(j,i)/10);
                    board(i,j).BackgroundColor = button_color;
                elseif loading_board(j,i) < 9 && loading_board(j,i) > 0
                    board(i,j).BackgroundColor = [0.8 0.8 0.8];
                    board(i,j).String = num2str(loading_board(j,i));
                end                
            end
        end
    end
    ...
    function save_game(hObject,~)
        save = 1;
        important_info = [length,height,mines,time,steps,str2num(flags_left.String),opened_squares];
        save_board = zeros(length,height);
        for i = 1:length
            for j = 1:height
                if strcmp(board(i,j).String,'?')
                    save_board(i,j) = -3;
                elseif board(i,j).BackgroundColor == [0,0,0]
                    save_board(i,j) = -2;
                elseif board(i,j).BackgroundColor == button_color                   
                    if strcmp(board(i,j).UserData,'BOMB')
                        save_board(i,j) = 9;
                    else
                        if ~str2num(board(i,j).UserData)
                            save_board(i,j) = -1;
                        else
                        save_board(i,j) = str2num(board(i,j).UserData)*10;
                        end
                    end
                elseif board(i,j).BackgroundColor == [0.8,0.8,0.8]
                    if isempty(board(i,j).String)
                        save_board(i,j) = 0;
                    else
                        save_board(i,j) = str2num(board(i,j).String);
                    end
                end
            end
        end
        tmp_board = flip(save_board,2);
        fixed_save_board = (tmp_board)';
        savegame_file = fopen('save_minesweeper.txt','wt');
        for i = 1:height
            for j = 1:length
                fprintf(savegame_file,'%i ',fixed_save_board(i,j));
            end
            fprintf(savegame_file,'\n');
        end
        fclose(savegame_file);
        game_info = fopen('ongoing_game.txt','wt');
        fprintf(game_info,'%u ',important_info);
        fclose(game_info);
        exit_game(hObject);
    end
    ...
    function exit_game(hObject,~)
        if ~save 
            erase_savegame = fopen('save_minesweeper.txt','w');
            fclose(erase_savegame);
            erase_ongoing_game = fopen('ongoing_game.txt','w');
            fclose(erase_ongoing_game);
        end
       
        game_options_file = fopen('game_options.txt','wt');
        options_stuff = [length,height,mines,display_animations_value,play_sound_value,show_tips_value,always_continue_saved_game_value,always_save_game_on_exit_value,question_marks];
        fprintf(game_options_file,'%u ',options_stuff);
    ...
    function new_game_selection(~,~)
        if steps ~= 0
            new_game_selection_figure = figure('position',[(f.Position(1)+f.Position(3))/2-100 (f.Position(2)+f.Position(4))/2-50 350 250],'numbertitle','off','name','New game', 'menubar', 'none');
            new_game_selection_label = uicontrol(new_game_selection_figure,'style','text','position',[10 190 330 50],'string','What do you want to do with the game in progress?','fontsize',16,'foregroundcolor','b');
            new_game_selection_new_game = uicontrol(new_game_selection_figure,'style','pushbutton','position',[10 130 330 50],'string','Quit and start a new game','fontsize',16,'callback',@start_new_game);
            new_game_selection_restart_game = uicontrol(new_game_selection_figure,'style','pushbutton','position',[10 70 330 50],'string','Restart this game','fontsize',16,'callback',@restart_game);
            new_game_selection_keep_playing = uicontrol(new_game_selection_figure,'style','pushbutton','position',[10 10 330 50],'string','Keep playing','fontsize',16,'callback',@closing_small_figure_request);
        end
    end
    ...
    function restart_game(hObject,~)
        ...
        for i = 1:length
            for j = 1:height
                ...
                board(i,j).BackgroundColor = button_color;
                if ~isempty(board(i,j).String)
                    board(i,j).UserData = board(i,j).String;
                    board(i,j).String = '';
                end
            end
        end
    ...
    function statistics(~,~)
        stats_figure = dialog('position',[(f.Position(1)+f.Position(3))/2-100 (f.Position(2)+f.Position(4))/2-50 382 391],'numbertitle','off','name','Statistics');
    end
    ...
    function game_options(~,~)
        ...
        choose_beginner = uicontrol(button_group,'style','radiobutton','position',[20 330 100 20],'string','Beginner 10 9x9','userdata','beginner');
        choose_intermediate = uicontrol(button_group,'style','radiobutton','position',[20 300 100 20],'string','Intermediate 40 16x16','userdata','intermediate');
        choose_advanced = uicontrol(button_group,'style','radiobutton','position',[20 270 100 20],'string','Advanced 99 16x30','userdata','advanced');
        choose_custom = uicontrol(button_group,'style','radiobutton','position',[260 340 100 20],'string','Custom','userdata','custom');
        label_height = uicontrol(options_figure,'style','text','position',[260 310 40 20],'string','Height (9-24):','enable','off');
        label_length = uicontrol(options_figure,'style','text','position',[260 280 40 20],'string','Length (9-30):','enable','off');
        label_mines = uicontrol(options_figure,'style','text','position',[260 250 40 20],'string','Mines (10-668):','enable','off');
        edit_height = uicontrol(options_figure,'style','edit','position',[330 310 40 20],'enable','off');
        edit_length = uicontrol(options_figure,'style','edit','position',[330 280 40 20],'enable','off');
        ...
        allow_question_marks = uicontrol(options_figure,'style','checkbox','position',[20 70 300 20],'string','Allow question marks (on double right-click)','value',1);
        options_ok_button = uicontrol(options_figure,'style','pushbutton','position',[140 15 110 25],'string','OK','callback',@change_game_option);
        ...
        if length == 9 && height == 9 && mines == 10
            choose_beginner.Value = 1;
            difficulty = 'beginner';
        elseif length == 16 && height == 16 && mines == 40
            choose_intermediate.Value = 1;
            difficulty = 'intermediate';
        elseif length == 30 && height == 16 && mines == 99
            choose_advanced.Value = 1;  
            difficulty = 'advanced';
        else
            choose_custom.Value = 1;
            edit_height.String = num2str(height);
            edit_length.String = num2str(length);
            edit_mines.String = num2str(mines);
            difficulty = 'custom';
            game_difficulty();
        end
        display_animations.Value = display_animations_value;
        play_sound.Value = play_sound_value;
        show_tips.Value = show_tips_value;
        always_continue.Value = always_continue_saved_game_value;
        always_save.Value = always_save_game_on_exit_value;
        allow_question_marks.Value = question_marks; 
        
        function game_difficulty(~,~)
            if choose_custom.Value
                label_height.Enable = 'on';
                label_length.Enable = 'on';
                label_mines.Enable = 'on';
                edit_height.Enable = 'on';
                edit_length.Enable = 'on';
                edit_mines.Enable = 'on';
            else
                label_height.Enable = 'off';
                label_length.Enable = 'off';
                label_mines.Enable = 'off';
                edit_height.Enable = 'off';
                edit_length.Enable = 'off';
    ...
    function change_game_option(~,~)
        beginner_height = 9;
        beginner_length = 9;
        beginner_mines = 10;
        intermediate_height = 16;
        intermediate_length = 16;
        intermediate_mines = 40;
        advanced_height = 16;
        advanced_length = 30;
        advanced_mines = 99;
        question_marks = allow_question_marks.Value;
        display_animations_value = display_animations.Value;
        play_sound_value = play_sound.Value;
        show_tips_value = show_tips.Value;
        always_continue_saved_game_value = always_continue.Value;
        always_save_game_on_exit_value = always_save.Value;
        
        if (strcmp(difficulty,choose_beginner.UserData) && choose_beginner.Value == 1) || (strcmp(difficulty,choose_intermediate.UserData) && choose_intermediate.Value == 1) || (strcmp(difficulty,choose_advanced.UserData) && choose_advanced.Value == 1)...
                || (choose_custom.Value == 1 && strcmp(difficulty,choose_custom.UserData) && str2num(edit_length.String) == length && str2num(edit_height.String) == height && str2num(edit_mines.String) == mines)
            closing_small_figure_request(options_ok_button);
        else
            delete(board)
            if choose_beginner.Value == 1
                length = 9;
                height = 9;
                mines = 10;
                difficulty = 'beginner';
            elseif choose_intermediate.Value == 1
                length = 16;
                height = 16;
                mines = 40;
                difficulty = 'intermediate';
            elseif choose_advanced.Value == 1
                length = 30;
                height = 16;
                mines = 99;
                difficulty = 'advanced';
            elseif choose_custom.Value == 1
                if ~isempty(str2num(edit_length.String)) && str2num(edit_length.String) > 9 && str2num(edit_length.String) < 30 ...
                        && ~isempty(str2num(edit_height.String)) && str2num(edit_height.String) > 9 && str2num(edit_height.String) < 24 ...
                        && ~isempty(str2num(edit_mines.String)) && str2num(edit_mines.String) > 10 && str2num(edit_mines.String) < str2num(edit_height.String) * str2num(edit_length.String) - 10
                    length = str2num(edit_length.String);
                    height = str2num(edit_height.String);
                    mines = str2num(edit_mines.String);
                    difficulty = 'custom';
                else
                    edit_length.String = '';
                    edit_height.String = '';
            ...
            f.Position = [200 500-(height+5)*button_size (length+3)*button_size (height+4)*button_size];
            stopwatch.Position = [button_size*length/4 button_size button_size*2.5 button_size*1.5];
            flags_left.Position = [button_size*length*3/4 button_size button_size*2.5 button_size*1.5];
            start_new_game(options_ok_button);
        end
    end
